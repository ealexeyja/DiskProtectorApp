...
if [ ! -d "./publish-test" ]; then
    ...
    echo "   Ejecuta primero './build-and-test.sh'"
    exit 1
fi
...
if [ -f "./publish-test/DiskProtectorApp.exe" ]; then
    cp ./publish-test/DiskProtectorApp.exe ./DiskProtectorApp-organized/
...
if ls ./publish-test/*.dll 1> /dev/null 2>&1; then
    mv ./publish-test/*.dll ./DiskProtectorApp-organized/libs/ 2>/dev/null
...
if [ -d "./publish-test/en" ]; then
    mv ./publish-test/en ./DiskProtectorApp-organized/locales/
    echo "✅ Recursos en inglés movidos"
fi

# Mover carpeta de español si existe
if [ -d "./publish-test/es" ]; then
    mv ./publish-test/es ./DiskProtectorApp-organized/locales/
    echo "✅ Recursos en español movidos"
fi

# Eliminar carpeta de alemán si existe
if [ -d "./publish-test/de" ]; then
    rm -rf ./publish-test/de
    echo "🗑️  Recursos en alemán eliminados"
fi

# Copiar archivos de configuración
if ls ./publish-test/*.json 1> /dev/null 2>&1; then
    mv ./publish-test/*.json ./DiskProtectorApp-organized/config/ 2>/dev/null
    echo "✅ Archivos JSON movidos a config/"
fi

if ls ./publish-test/*.config 1> /dev/null 2>&1; then
    mv ./publish-test/*.config ./DiskProtectorApp-organized/config/ 2>/dev/null
...
echo ""
echo "✅ Estructura organizada creada en ./DiskProtectorApp-organized/"
