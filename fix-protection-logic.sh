...
namespace DiskProtectorApp.Services
{
    public class DiskService
    {
        ...
        /// 
        /// DISCO DESPROTEGIDO:
        /// - Usuarios TIENE permisos básicos Y AuthUsers TIENE modificación/escritura
        /// 
        /// DISCO PROTEGIDO:
        /// - Usuarios NO TIENE permisos (ni básicos) Y AuthUsers SOLO TIENE básicos
        /// </summary>
        private bool IsDriveProtected(string drivePath)
        {
            try
            {
                ...
                bool authUsersHasOnlyBasicPermissions = false;
                ...
                foreach (FileSystemAccessRule rule in rules)
                {
                        ...
                        rule.AccessControlType == AccessControlType.Allow)
                    {
                        ...
                        // Verificar si solo tiene permisos básicos (lectura y ejecución, mostrar contenido de carpeta, lectura)
                        if ((rights & (FileSystemRights.ReadAndExecute | FileSystemRights.ListDirectory | FileSystemRights.Read)) != 0 &&
                            (rights & (FileSystemRights.Modify | FileSystemRights.Write)) == 0)
                        {
                            authUsersHasOnlyBasicPermissions = true;
                            LogMessage($"[PERMISSION_CHECK] Usuarios autenticados solo tiene permisos básicos", "DEBUG");
                        }
                    }
                }
                ...
                // - Protegido: !usersHasBasicPermissions && authUsersHasOnlyBasicPermissions
                
                bool isUnprotected = usersHasBasicPermissions && authUsersHasModifyWritePermissions;
                bool isProtected = !usersHasBasicPermissions && authUsersHasOnlyBasicPermissions;
                
                LogMessage($"[PERMISSION_CHECK] Resultado detección:", "INFO");
                LogMessage($"[PERMISSION_CHECK]   Usuarios(Básicos:{usersHasBasicPermissions}) AuthUsers(Mod/Esc:{authUsersHasModifyWritePermissions}) AuthUsers(SoloBásicos:{authUsersHasOnlyBasicPermissions})", "INFO");
