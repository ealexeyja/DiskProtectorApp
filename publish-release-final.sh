...
if [ ! -f "./DiskProtectorApp-v$CURRENT_VERSION.tar.gz" ]; then
    ...
    echo "   Ejecuta './finalize-release-final.sh' primero"
    exit 1
fi
...
git commit -m "release: v$CURRENT_VERSION - Actualización a .NET 8.0"

echo "3️⃣ Subiendo cambios a GitHub..."
git push origin main

echo "4️⃣ Creando tag v$CURRENT_VERSION..."
git tag -a v$CURRENT_VERSION -m "Release v$CURRENT_VERSION - Actualización a .NET 8.0"
