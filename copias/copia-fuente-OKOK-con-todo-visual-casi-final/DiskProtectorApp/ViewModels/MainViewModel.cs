...
namespace DiskProtectorApp.ViewModels
{
    public class MainViewModel : INotifyPropertyChanged
    {
        ...
        private async Task ExecuteProtectDisks()
        {
            ...
            var result = MessageBoxWindow.ShowDialog($"¿Está seguro que desea proteger {selectedDisks.Count} disco(s)?", "Confirmar protección", MessageBoxButton.YesNo, ownerWindow);
        ...
        private async Task ExecuteUnprotectDisks()
        {
            ...
            // Reemplazar MessageBox.Show con ventana personalizada
            var ownerWindow = Application.Current.MainWindow;
            var result = MessageBoxWindow.ShowDialog($"¿Está seguro que desea desproteger {selectedDisks.Count} disco(s)?", "Confirmar desprotección", MessageBoxButton.YesNo, ownerWindow);
            if (result != MessageBoxResult.Yes) return;
