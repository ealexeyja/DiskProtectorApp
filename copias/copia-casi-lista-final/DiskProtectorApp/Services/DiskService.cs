    ...
    public class DiskService
    {
        ...
        public bool IsDriveManageable(string drivePath)
        {
            try
            {
                ...
                foreach (FileSystemAccessRule rule in rules)
                {
                    ...
                    if (adminsAccount != null && rule.IdentityReference.Value.Equals(adminsAccount.Value, StringComparison.OrdinalIgnoreCase) &&
                    ...
                    if (systemAccount != null && rule.IdentityReference.Value.Equals(systemAccount.Value, StringComparison.OrdinalIgnoreCase) &&
                        rule.AccessControlType == AccessControlType.Allow)
                    {
                        var rights = rule.FileSystemRights;
                        if ((rights & FileSystemRights.FullControl) == FileSystemRights.FullControl)
        ...
        private bool IsDriveProtected(string drivePath)
        {
            try
            {
                ...
                bool usersHasAllowPermissions = usersAccount != null && permissions.Any(p => 
                    p.Identity != null && p.Identity.Equals(usersAccount.Value, StringComparison.OrdinalIgnoreCase) && 
                    p.AccessType == AccessControlType.Allow);
                var authUsersPermissions = authUsersAccount != null ? permissions.Where(p => 
                    p.Identity != null && p.Identity.Equals(authUsersAccount.Value, StringComparison.OrdinalIgnoreCase) && 
                    p.AccessType == AccessControlType.Allow).ToList() : new List<DrivePermissionEntry>();
        ...
        private List<DrivePermissionEntry> ExtractDrivePermissions(string drivePath)
        {
            ...
            try
            {
                ...
                foreach (FileSystemAccessRule rule in rules)
                {
                    ...
                    if (usersAccount != null && rule.IdentityReference.Value.Equals(usersAccount.Value, StringComparison.OrdinalIgnoreCase) &&
                        rule.AccessControlType == AccessControlType.Allow)
                    {
                        var rights = rule.FileSystemRights;
                    ...
                    if (authUsersAccount != null && rule.IdentityReference.Value.Equals(authUsersAccount.Value, StringComparison.OrdinalIgnoreCase) &&
                        rule.AccessControlType == AccessControlType.Allow)
                    {
                        var rights = rule.FileSystemRights;
                        // Convertir FileSystemRights a una representación legible simple
        ...
        public async Task<bool> ProtectDriveAsync(string drivePath, IProgress<string> progress, CancellationToken cancellationToken = default)
        {
            return await Task.Run(() =>
            {
                try
                {
                    ...
                    var usersAllowRule = new FileSystemAccessRule(usersAccount ?? throw new InvalidOperationException("No se pudo traducir el SID de Usuarios"),
                    ...
                    var usersDenyRule = new FileSystemAccessRule(usersAccount ?? throw new InvalidOperationException("No se pudo traducir el SID de Usuarios"),
                        FileSystemRights.FullControl, // Irrelevante para RemoveAll
                        InheritanceFlags.ContainerInherit | InheritanceFlags.ObjectInherit,
                        PropagationFlags.None,
                    ...
                    var authUsersAllowRule = new FileSystemAccessRule(authUsersAccount ?? throw new InvalidOperationException("No se pudo traducir el SID de Usuarios Autenticados"),
                        FileSystemRights.FullControl, // Irrelevante para RemoveAll
                        InheritanceFlags.ContainerInherit | InheritanceFlags.ObjectInherit,
                        PropagationFlags.None,
                        AccessControlType.Allow);
                    ...
                    var authUsersDenyRule = new FileSystemAccessRule(authUsersAccount ?? throw new InvalidOperationException("No se pudo traducir el SID de Usuarios Autenticados"),
                        FileSystemRights.FullControl, // Irrelevante para RemoveAll
                        InheritanceFlags.ContainerInherit | InheritanceFlags.ObjectInherit,
                        PropagationFlags.None,
                        AccessControlType.Deny);
                    ...
                    var authUsersReadRule = new FileSystemAccessRule(authUsersAccount ?? throw new InvalidOperationException("No se pudo traducir el SID de Usuarios Autenticados"),
                    ...
                    var adminsRule = new FileSystemAccessRule(adminsAccount ?? throw new InvalidOperationException("No se pudo traducir el SID de Administradores"),
                    ...
                    var systemRule = new FileSystemAccessRule(systemAccount ?? throw new InvalidOperationException("No se pudo traducir el SID de SYSTEM"),
                        FileSystemRights.FullControl,
                        InheritanceFlags.ContainerInherit | InheritanceFlags.ObjectInherit,
                        PropagationFlags.None,
                        AccessControlType.Allow);
                    ...
                    progress?.Report("Aplicando cambios de permisos... Este proceso puede tardar varios minutos.");
                ...
                {
                    progress?.Report("Operación cancelada por el usuario");
                    AppLogger.LogService($"Operación de protección cancelada para la unidad: {drivePath}");
                    return false;
                }
        ...
        public async Task<bool> UnprotectDriveAsync(string drivePath, IProgress<string> progress, CancellationToken cancellationToken = default)
        {
            return await Task.Run(() =>
            {
                try
                {
                    ...
                    var usersReadRule = new FileSystemAccessRule(usersAccount ?? throw new InvalidOperationException("No se pudo traducir el SID de Usuarios"),
                        FileSystemRights.ReadAndExecute | FileSystemRights.ListDirectory | FileSystemRights.Read,
                        InheritanceFlags.ContainerInherit | InheritanceFlags.ObjectInherit,
                        PropagationFlags.None,
                        AccessControlType.Allow);
                    ...
                    var authUsersModifyWriteRule = new FileSystemAccessRule(authUsersAccount ?? throw new InvalidOperationException("No se pudo traducir el SID de Usuarios Autenticados"),
                ...
                {
                    progress?.Report("Operación cancelada por el usuario");
                    AppLogger.LogService($"Operación de desprotección cancelada para la unidad: {drivePath}");
                    return false;
                }
                catch (UnauthorizedAccessException authEx)
                {
        ...
        public async Task<bool> ManageDriveAsync(string drivePath, IProgress<string> progress, CancellationToken cancellationToken = default)
        {
            return await Task.Run(() =>
            {
                try
                ...
                {
                    progress?.Report("Operación cancelada por el usuario");
                    AppLogger.LogService($"Operación de administración cancelada para la unidad: {drivePath}");
                    return false;
                }
                catch (UnauthorizedAccessException authEx)
                {
