...
namespace DiskProtectorApp.ViewModels
{
    public class MainViewModel : INotifyPropertyChanged
    {
        ...
        private ObservableCollection<DiskInfo> _selectedDisks = new();
        ...
        // Nuevo: Comando para administrar discos no administrables
        public RelayCommand ManageDriveCommand { get; private set; }
        ...
        public MainViewModel()
        {
            ...
            ProtectCommand = new RelayCommand(async (object? _) => await ExecuteProtectDisksAsync(), (object? _) => CanExecuteProtect());
            UnprotectCommand = new RelayCommand(async (object? _) => await ExecuteUnprotectDisksAsync(), (object? _) => CanExecuteUnprotect());
            ...
            // Comando para administrar discos no administrables
            ManageDriveCommand = new RelayCommand(async (object? _) => await ExecuteManageDriveAsync(), (object? _) => CanExecuteManageDrive());
        ...
        {
            AppLogger.LogViewModel("ExecuteProtectDisksAsync iniciado");
        ...
        {
            AppLogger.LogViewModel("ExecuteUnprotectDisksAsync iniciado");
        ...
        /// </summary>
        private async Task ExecuteManageDriveAsync()
        {
            AppLogger.LogViewModel("ExecuteManageDriveAsync iniciado");
        ...
        {
            AppLogger.LogViewModel("ExecuteRefreshDisks llamado");
            RefreshDisks();
        }

        private void ExecuteExit()
        {
            AppLogger.LogViewModel("ExecuteExit llamado");
            if (IsWorking)
            {
        ...
        {
            AppLogger.LogViewModel("ExecuteShowInfo llamado");
        ...
        // Métodos CanExecute para comandos - LÓGICA CORREGIDA DE PRIORIDADES
        private bool CanExecuteProtect()
        {
            // Activar solo si:
            // 1. No hay operaciones en curso (IsWorking = False)
            // 2. Hay al menos un disco seleccionado (IsSelected = True)
            // 3. Ese disco es Seleccionable (IsSelectable = True) <- Prioridad 1
            ...
            AppLogger.LogViewModel($"CanExecuteProtect: {canExecute} - IsWorking: {IsWorking}, SelectedAndSelectableAndManageableAndUnprotectedCount: {Disks.Count(d => d.IsSelected && d.IsSelectable && d.IsManageable && !d.IsProtected)}");
            return canExecute;
        }
        ...
        private bool CanExecuteUnprotect()
        {
            ...
            AppLogger.LogViewModel($"CanExecuteUnprotect: {canExecute} - IsWorking: {IsWorking}, SelectedAndSelectableAndManageableAndProtectedCount: {Disks.Count(d => d.IsSelected && d.IsSelectable && d.IsManageable && d.IsProtected)}");
            return canExecute;
        }
        ...
        /// </summary>
        private bool CanExecuteManageDrive()
        {
            // Activar solo si:
            // 1. No hay operaciones en curso (IsWorking = False)
            // 2. Hay al menos un disco seleccionado (IsSelected = True)
            // 3. Ese disco es Seleccionable (IsSelectable = True) <- Prioridad 1
            // 4. Ese disco NO es Administrable (IsManageable = False) <- Prioridad 2 (Máxima)
            
            bool canExecute = !IsWorking && 
                             Disks.Any(d => d.IsSelected && d.IsSelectable && !d.IsManageable);
                             
            AppLogger.LogViewModel($"CanExecuteManageDrive: {canExecute} - IsWorking: {IsWorking}, SelectedAndSelectableAndNotManageableCount: {Disks.Count(d => d.IsSelected && d.IsSelectable && !d.IsManageable)}");
            return canExecute;
        }
        ...
        private void UpdateCommandStates()
        {
            ...
            // Actualizar estado del comando de administración
            ManageDriveCommand?.RaiseCanExecuteChanged();
        }

        // Implementación de INotifyPropertyChanged
        public event PropertyChangedEventHandler? PropertyChanged; // <-- Corrección de nullability
